...
export const actions = {
    async getProductData({ commit, state, dispatch }) {
        return new Promise(function (resolve, reject) {
            let endpoint = _.join([_.trim('/products', '/'), _.trim(state.productId, '/')], '/');
    ...
    async getRelatedProducts({ commit, dispatch }, payload) {
        return new Promise(function (resolve, reject) {
            let endpoint = _.join(
                [_.trim('/products', '/'), _.trim(payload.data, '/'), _.trim('/relations', '/')],
                '/'
            );
    ...
    async getProductsCrossBuybox({ commit, dispatch }, payload) {
        return new Promise((resolve, reject) => {
            let endpoint = _.join([_.trim('/products', '/'), _.trim(payload.data, '/'), _.trim('/buybox', '/')], '/');
    ...
    async getProductsCrossByOrder({ commit, dispatch }, payload) {
        return new Promise((resolve, reject) => {
            let endpoint = _.join([_.trim('/products', '/'), _.trim(payload.data, '/'), _.trim('/byorder', '/')], '/');
    ...
    async getProductsCrossSimilar({ commit, dispatch }, payload) {
        return new Promise((resolve, reject) => {
            let endpoint = _.join([_.trim('/products', '/'), _.trim(payload.data, '/'), _.trim('/similar', '/')], '/');
