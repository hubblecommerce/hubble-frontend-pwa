        ...
        return new Promise((resolve, reject) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (payload.data.result.item.category_id) {
                    let endpoint = _.join(
                        [_.trim('/categories', '/'), _.trim(payload.data.result.item.category_id, '/')],
                        '/'
                    );
                    ...
                    dispatch(
                        'apiCall',
                        {
                            ...
                            params: {
                                _withProps: _.join(
                                    ['path', 'path_ids', 'path_names', 'path_urls', 'display_mode', 'parent_id'],
                                    ','
                                ),
                            },
                        },
                ...
                if (payload.data.result.item.product_id) {
                    let endpoint = _.join(
                        [_.trim('/products', '/'), _.trim(payload.data.result.item.product_id, '/')],
                        '/'
                    );
                    ...
                    dispatch(
                        'apiCall',
                        {
                            ...
                            params: _.merge({}, query, {
                                _withProps: _.join(
                                    ['facets', 'media_gallery', 'search_result_data_children', 'status'],
                                    ','
                                ),
                            }),
                        },
                ...
                if (payload.data.result.item.content_id) {
                    let endpoint = _.join(
                        [_.trim('/cms/blocks', '/'), _.trim(payload.data.result.item.request_path, '/')],
                        '/'
                    );
