...
fse.exists(path.join(projectDir, '/store/initialStore.js'), function (exists) {
    if (!exists) {
        fse.copy(path.join(projectDir, '/node_modules/@hubblecommerce/hubble/initialStore.js'), path.join(projectDir, '/store/initialStore.js'));
    }
});

fse.exists(path.join(projectDir, '/.env'), function (exists) {
    if (!exists) {
        fse.copy(path.join(projectDir, '/node_modules/@hubblecommerce/hubble/.env_example'), path.join(projectDir, '/.env'));
    }
});
