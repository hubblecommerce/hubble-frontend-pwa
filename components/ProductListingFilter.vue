<template>
  <div class="filter-wrp">
    ...
    <button v-if="$mq === 'sm' || $mq === 'md'" class="button button-primary open-filter" @click="toggle()">
      ...
      <span v-text="$t('Filter')"></span>
      <material-ripple/>
    </button>

    <transition-expand-layer v-if="$mq === 'sm' || $mq === 'md'" :rightLeft="true">
      <div v-if="showFilters" class="transition-expand-wrp">
        <div class="container position-static">
          ...
          <div class="row overlay-header">
            ...
            <div class="overlay-headline" v-text="$t('Filter')"></div>
          </div>
          ...
          <div class="facets-wrp">
            ...
            <div class="facet-wrp">
              <collapsible :toggle-text="$t('Price')" :max-height="100" open-icon-class="icon-chevron-down" close-icon-class="icon-chevron-up">
                <price-slider
          ...
          <div class="apply-filter-wrp">
            <button class="button button-primary apply-filter" @click="applyFilter()">
              <span v-text="$t('Apply & Close')"></span>
              <material-ripple/>
            </button>
          </div>
    ...
    <div v-if="$mq === 'lg'" class="facets-wrp desktop">
      ...
      <div class="facet-wrp">
        <collapsible-filter :class="{active: hasPriceFacetsSelected}" :toggle-text="$t('Price')" :max-height="200" open-icon-class="icon-chevron-down" close-icon-class="icon-chevron-up">
          <price-slider
              :data-min-value="minPriceSelectable"
              :data-max-value="maxPriceSelectable"
              :data-min-selected="minPriceSelected"
    ...
    <div v-if="hasFacetsSelected && $mq === 'lg'" class="selected-filters">
      <div class="selected-label" v-text="$t('Your choice:')"></div>
      <div v-for="(facet, facetIndex) in requestStringFacets"
          v-if="facet.selected"
          :key="facetIndex"
          class="filter">
      ...
      <div v-if="hasPriceFacetsSelected" class="filter">
        <button class="button button-secondary" @click="routeOnPropertyRemove('price')">
          {{ $t('price') }} <i class="icon icon-close"/>
          <material-ripple/>
        </button>
      </div>
      <div class="reset-label" @click="routeOnPropertyRemoveAll()" v-text="$t('Reset all')"></div>
    </div>
...
import SelectableFacet from './SelectableFacet.vue'
import SelectableLimit from './SelectableLimit.vue'
import SelectableOrder from './SelectableOrder.vue'
...
export default {
  ...
  data() {
    return {
      ...
      queryWellKnown: ['term', 'dir', 'order', 'limit'],
  ...
  computed: {
    ...
    hasFacetsSelected() {
      return this.hasNumberFacetsSelected || this.hasStringFacetsSelected || this.hasPriceFacetsSelected;
    },
  ...
  created() {
    ...
    this.$bus.$on('selectable-order-changed', response => {
      ...
      this.$store.commit('modApiRequests/setSelectedQueryParam', {
        name: 'dir',
        data: this.getSortingDirection(response.payload.data)
      })
  ...
  methods: {
    ...
    getSelectedFacetOptionsLabel(facet) {
      let _id = facet.key // e.g. 'color', 'size'
    ...
    routeOnPropertyChange() {
      ...
      return new Promise((resolve, reject) => {
        ...
        // always act on string facets based on product items
