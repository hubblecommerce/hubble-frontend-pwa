...
export default {
  ...
  methods: {
    ...
    doSearch: _.debounce(function() {
      ...
      let _endpoint = '/rocket-api/json/' + this.locale + '/search/autocomplete'
    ...
    groupResults() {
      var _group_l = { name: 'left', items: [], count: 0 }
      var _group_r = { name: 'right', items: [], count: 0 }

      // group categories
      var _group_categories = this.items.filter(
        item => item.meta.name === 'catalog_category'
      )

      if (!_.isEmpty(_group_categories)) {
        var _cnt_items_group = _.size(_group_categories[0].items)
      ...
      // group pages
      var _group_pages = this.items.filter(
        item => item.meta.name === 'cms_page'
      )

      if (!_.isEmpty(_group_pages)) {
        var _cnt_items_group = _.size(_group_pages[0].items)
        _group_l.count += _cnt_items_group
      ...
      // group products
      var _group_products = this.items.filter(
        item => item.meta.name === 'catalog_product'
      )

      if (!_.isEmpty(_group_products)) {
        var _cnt_items_group = _.size(_group_products[0].items)
    ...
    sendStats(data) {
      ...
      var _route = route('utilities.stats')
