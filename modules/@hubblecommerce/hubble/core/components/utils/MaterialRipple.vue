<template>
    <div class="material-ripple__component" :class="computedClasses" :style="computedStyles" />
</template>
    ...
    methods: {
        ...
        animate(event) {
            ...
            if (this.center) {
                ...
                const inputX = event.type === 'click' ? event.pageX : event.originalEvent.touches[0].pageX;
                const inputY = event.type === 'click' ? event.pageY : event.originalEvent.touches[0].pageY;
                const center = size / 2;
                finalSize = size;
                top = inputY - position.top - center;
                left = inputX - position.left - center;
            }
            this.setInnerStyles(finalSize, top, left);
        },
