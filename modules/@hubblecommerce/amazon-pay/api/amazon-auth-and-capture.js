...
const response = function(req, res, next) {
    ...
    let authorizationReferenceId = query.orderId;

    // Optional fields
    let paymentAction = "Authorize";
    ...
    parameters['AuthorizationAmount.Amount'] = amount; // Mandatory
    parameters['AuthorizationAmount.CurrencyCode'] = currency; // Mandatory
    parameters.AuthorizationReferenceId = authorizationReferenceId; // Mandatory
    parameters.CaptureNow = true; // Mandatory
    ...
    parameters.TransactionTimeout = 0; // Mandatory
    parameters.Version = '2013-01-01'; // Mandatory

    delete parameters.AuthorizationAmount;
    ...
    axios({
        ...
        url: 'https://mws-eu.amazonservices.com/OffAmazonPayments'+sandboxMode+'/2013-01-01?' +
            ...
            '&AuthorizationAmount.Amount='+ parameters['AuthorizationAmount.Amount'] +
            '&AuthorizationAmount.CurrencyCode='+ parameters['AuthorizationAmount.CurrencyCode'] +
            '&AuthorizationReferenceId='+ parameters.AuthorizationReferenceId +
            '&CaptureNow='+ parameters.CaptureNow +
...
export default {
    path: '/api/amazon-auth-and-capture',
    handler: response
}
